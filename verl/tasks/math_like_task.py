...
class MathLikeTaskConfig(TaskConfig):
    ...
    pass_rate_max: float = 1.0
    pass_rate_key: str = "pass_rate"
...
class MathLikeTask(Task):
    ...
    def make_map_fn(self, split: Split) -> Callable[[dict, int], dict]:
        ...
        def process_fn(example, idx):
            ...
            enclosed_with_parentheses = "\\(" in answer and "\\)" in answer
    ...
    def make_filter_fn(self, split: Split) -> Optional[Callable[[dict], bool]]:
        pass_rate_key = self.config.pass_rate_key
        pass_rate_min = self.config.pass_rate_min
        pass_rate_max = self.config.pass_rate_max

        def _keep_non_empty_answers(x):
            return x["reward_model"]["ground_truth"] is not None and len(x["reward_model"]["ground_truth"]) > 0

        def _keep_pass_rate_threshold(x):
            return x[pass_rate_key] >= pass_rate_min and x[pass_rate_key] <= pass_rate_max

        filter_fns = []
        if self.config.filter_empty_answers:
            filter_fns.append(_keep_non_empty_answers)
        if pass_rate_min > 0 or pass_rate_max < 1:
            filter_fns.append(_keep_pass_rate_threshold)

        if len(filter_fns) > 0:
            return lambda x: all(fn(x) for fn in filter_fns)
        else:
            return None
